...
test('点歌榜提供已点已唱及歌曲歌手切换并由站主批量唱完', () => {
  ...
  assert.match(station, /data-request-ranking-panel[^>]*className="relative flex min-h-\[calc\(100svh-12rem\)\] flex-col/)
  assert.match(station, /rankingView === 'requests' \? \(\s*<div className="flex flex-1 flex-col gap-4">/)
  assert.match(station, /<div className="mt-auto flex justify-center pt-6">/)
...
test('猜歌榜可切换用户得分榜和歌曲正确率榜', () => {
  ...
  assert.match(station, /useState<'participants' \| 'songs'>\('songs'\)/)
  assert.match(station, /data-quiz-ranking-controls/)
...
test('路演基本信息四项同排并把地点限制为三个固定选项', async () => {
  const source = readFileSync(roadshowPanelUrl, 'utf8')
  const { ROADSHOW_LOCATIONS } = await loadRoadshowModule()

  assert.deepEqual(ROADSHOW_LOCATIONS, ['医大（武鸣）', '医大（本部）', '南湖'])
