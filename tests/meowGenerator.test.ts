...
test('stores the normalized happiness message, date, and nickname atomically', () => {
  const entries = new Map<string, string>();
  const storage = { setItem: (key: string, value: string) => entries.set(key, value) };

  assert.deepEqual(storeHappinessContext(storage, '  跑步听歌  ', '2026-08-18T13:39:00+08:00', ' 小王 '), {
    message: '跑步听歌',
...
test('replaces message and date together when different stars are selected', () => {
  const stored: Array<{ message: string; createdAt: string; nickname: string }> = [];
  ...
  openHappinessMeowGenerator({ message: '第一颗星', createdAt: '2026-08-18T08:00:00Z', nickname: '小王', getStorage: () => storage, navigate });
  openHappinessMeowGenerator({ message: '第二颗星', createdAt: '2026-08-19T09:00:00Z', nickname: '小李', getStorage: () => storage, navigate });

  assert.deepEqual(stored, [
    { message: '第一颗星', createdAt: '2026-08-18T08:00:00Z', nickname: '小王' },
    { message: '第二颗星', createdAt: '2026-08-19T09:00:00Z', nickname: '小李' },
  ]);
