...
test('游客只读取站主未删除谱页，不泄露歌词和私人字段', async () => {
  ...
  const score = { id: 'score-test', songId: 'test', songTitle: '测试', songArtist: '歌手', pages: ['https://example.com/score.jpg'], updatedAt: '2026-09-16', lyrics: '私人歌词', workspaceId: ownerId, privateNote: 'private' };
  ...
  for (const key of ['lyrics', 'workspaceId', 'privateNote']) assert.equal(key in result.scores[0], false);
});
