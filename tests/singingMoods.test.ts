...
test('six singing moods can be saved together in browser and cloud', () => {
  const singingMoods = ['欢快', '感动', '想哭', '爽歌', '音色', '歌词'];
...
test('old singing moods normalize and deduplicate without losing selections', () => {
  assert.deepEqual(normalizeSingingMoods(['快乐', '欢快', '舒服', '音色', '歌词']), ['欢快', '音色', '歌词']);
  const result = validateRequest({ action: 'songRecords:save', alias: 'test', password: '123456', record: { ...record, singingMoods: ['快乐', '舒服'] } });
  assert.deepEqual(result.record.singingMoods, ['欢快', '音色']);
});
