...
test('管理员模式允许删除任意用户的星星且删除函数保留权限校验', () => {
  ...
  assert.match(source, /const canDeleteStar = \(star: StarData\) => !isLifeSeedStar\(star\) && \(star\.userId === userId \|\| isAdminDevice\)/)
  assert.match(source, /if \(!starToDelete\) return/)
  assert.match(source, /if \(isLifeSeedStar\(starToDelete\)\) return/)
