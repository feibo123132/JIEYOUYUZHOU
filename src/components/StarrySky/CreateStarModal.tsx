...
const CreateStarModal: React.FC<CreateStarModalProps> = ({ open, onClose, onConfirm, defaultColor = '#FFD700', allowSfx = true }) => {
      ...
      >
        ...
        <div className="space-y-6">
          ...
          <div className="flex space-x-3 pt-2 sticky bottom-0 bg-white/90 backdrop-blur-sm">
            ...
            <button
              onClick={() => { if (!allowSfx) { onConfirm({ color, size, shape, message }); return; } const bg = (window as any).__bgAudio as HTMLAudioElement | undefined; const ensure = (window as any).__ensureBgAudioGraph as (() => Promise<void>) | undefined; if (ensure) { try { ensure(); } catch {} } const gain = (window as any).__bgGain as any; if (bg || gain) { if ((window as any).__bgAudioRampTimer) { clearInterval((window as any).__bgAudioRampTimer); (window as any).__bgAudioRampTimer = null; } if (gain) { try { gain.gain.value = 0.2; } catch {} } else if (bg) { (window as any).__bgAudioOriginalVolume = (window as any).__bgAudioOriginalVolume ?? bg.volume; bg.volume = Math.max(0, ((window as any).__bgAudioOriginalVolume) * 0.2); } } const inc = () => { (window as any).__sfxPlayingCount = ((window as any).__sfxPlayingCount || 0) + 1; }; const rampUp = () => { const g = (window as any).__bgGain as any; if (g) { const from = g.gain.value; const to = 1; const steps = 20; const duration = 1000; let i = 0; if ((window as any).__bgAudioRampTimer) { clearInterval((window as any).__bgAudioRampTimer); } (window as any).__bgAudioRampTimer = setInterval(() => { i++; const t = i / steps; const v = from + (to - from) * t; try { g.gain.value = Math.min(1, Math.max(0, v)); } catch {} if (i >= steps) { clearInterval((window as any).__bgAudioRampTimer); (window as any).__bgAudioRampTimer = null; } }, Math.max(10, Math.floor(duration / steps))); return; } if (!bg) return; const fromV = bg.volume; const toV = 1; const stepsV = 20; const durationV = 1000; let j = 0; if ((window as any).__bgAudioRampTimer) { clearInterval((window as any).__bgAudioRampTimer); } (window as any).__bgAudioRampTimer = setInterval(() => { j++; const t = j / stepsV; const v = fromV + (toV - fromV) * t; bg.volume = Math.min(1, Math.max(0, v)); if (j >= stepsV) { clearInterval((window as any).__bgAudioRampTimer); (window as any).__bgAudioRampTimer = null; } }, Math.max(10, Math.floor(duration / stepsV))); }; const dec = () => { const c = ((window as any).__sfxPlayingCount || 0) - 1; (window as any).__sfxPlayingCount = c < 0 ? 0 : c; if ((window as any).__sfxPlayingCount === 0) { rampUp(); } }; const s = new Audio(getPublicUrl('点亮星星的音效.mp3')); s.currentTime = 0; inc(); s.addEventListener('ended', dec); s.addEventListener('error', dec); s.play().catch(dec); const list = ['朝着自己的月亮走.mp3','欢迎你到解忧宇宙遨游.mp3','每当宇宙闪烁.mp3','你本就是万千色彩.mp3','你终将会找到属于自己的月亮②.mp3','生活所有的无奈.mp3','愿你开心每一天.mp3','愿世界待你以温柔.mp3','月亮终究会到来.mp3','祝你有美好的一天.mp3']; const pick = list[Math.floor(Math.random()*list.length)]; const a = new Audio(getPublicUrl(pick)); a.currentTime = 0; inc(); a.addEventListener('ended', dec); a.addEventListener('error', dec); a.play().catch(dec); onConfirm({ color, size, shape, message }); }}
