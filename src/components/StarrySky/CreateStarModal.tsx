      ...
      >
        ...
        <div className="space-y-6">
          ...
          <div className="flex space-x-3 pt-2 sticky bottom-0 bg-white/90 backdrop-blur-sm">
            ...
            <button
              onClick={() => { const bg = (window as any).__bgAudio as HTMLAudioElement | undefined; if (bg) { if ((window as any).__bgAudioRampTimer) { clearInterval((window as any).__bgAudioRampTimer); (window as any).__bgAudioRampTimer = null; } (window as any).__bgAudioOriginalVolume = (window as any).__bgAudioOriginalVolume ?? bg.volume; bg.volume = Math.max(0, ((window as any).__bgAudioOriginalVolume) * 0.2); } if ((window as any).__initSfx) { try { (window as any).__initSfx(); } catch {} } const inc = () => { (window as any).__sfxPlayingCount = ((window as any).__sfxPlayingCount || 0) + 1; }; const rampUp = () => { if (!bg) return; const from = bg.volume; const to = 1; const steps = 20; const duration = 1000; let i = 0; if ((window as any).__bgAudioRampTimer) { clearInterval((window as any).__bgAudioRampTimer); } (window as any).__bgAudioRampTimer = setInterval(() => { i++; const t = i / steps; const v = from + (to - from) * t; bg.volume = Math.min(1, Math.max(0, v)); if (i >= steps) { clearInterval((window as any).__bgAudioRampTimer); (window as any).__bgAudioRampTimer = null; } }, Math.max(10, Math.floor(duration / steps))); }; const dec = () => { const c = ((window as any).__sfxPlayingCount || 0) - 1; (window as any).__sfxPlayingCount = c < 0 ? 0 : c; if ((window as any).__sfxPlayingCount === 0) { rampUp(); } }; const m = (window as any).__sfxMap as Record<string, HTMLAudioElement> | undefined; const fixedName = '点亮星星的音效.mp3'; const fixed = m?.[fixedName] || new Audio(getPublicUrl(fixedName)); fixed.currentTime = 0; inc(); fixed.addEventListener('ended', dec, { once: true }); fixed.addEventListener('error', dec, { once: true }); fixed.play().catch(dec); const files = ((window as any).__sfxFiles as string[] || []).filter(n => n !== fixedName && n !== '你终将会找到属于自己的月亮.mp3'); const pick = files.length ? files[Math.floor(Math.random() * files.length)] : null; if (pick) { const rand = m?.[pick] || new Audio(getPublicUrl(pick)); rand.currentTime = 0; inc(); rand.addEventListener('ended', dec, { once: true }); rand.addEventListener('error', dec, { once: true }); rand.play().catch(dec); } onConfirm({ color, size, shape, message }); }}
