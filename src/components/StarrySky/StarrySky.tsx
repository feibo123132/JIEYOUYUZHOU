...
const StarrySky: React.FC<StarrySkyProps> = ({ userNickname, onBack, userId }) => {
  ...
  const [draft, setDraft] = useState<{ color: string; size: number; shape: string; message: string }>({
    color: '#FFD700',
    size: 24,
    shape: 'star',
    message: ''
            ...
            <div className="text-center space-y-4">
              ...
              <div>
                <h3 className="text-xl font-bold text-gray-800 mb-2">
                  {selectedStar.nickname} 的星星
                </h3>
                <p className="text-gray-600 text-sm">
                  点亮时间: {formatTime(selectedStar.createdAt)}
                </p>
                {selectedStar.message && (
                  <p className="text-gray-700 text-sm mt-2">{selectedStar.message}</p>
                )}
              </div>
              ...
              <div className="flex space-x-3">
                ...
                {selectedStar.userId === userId && (
                  <button
                    ...
                    className="flex-1 bg-red-500 hover:bg-red-600 text-white py-2 px-4 rounded-lg transition-colors duration-200 flex items-center justify-center space-x-2"
                  >
