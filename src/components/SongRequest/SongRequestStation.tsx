...
import {
  ...
  parseArtistSettingsSnapshot, rebaseArtistSettingsDraft, resolveArtistSettingsPull, resolveSuccessfulArtistSettingsPush,
  saveArtistSettingsCache, saveArtistSettingsDraft,
  type ArtistSettingsPayload,
} from './artistSettings';
import {
  countQuizSongs, groupQuizSongs, parseQuizAssignments, QUIZ_LEVELS, selectQuizSongsForRoadshow, setQuizLevel,
...
type SungStatsSource = 'requests' | 'roadshow';
type SungRankingMode = 'artists' | 'songs';
...
const maskSongRecordAccount = (alias: string) => {
...
const ARTIST_AVATARS: Record<string, { src: string; position: string; scale: number }> = {
  周杰伦: { src: artistAvatarUrl('jay-chou.webp'), position: '50% 24%', scale: 1.35 },
  林俊杰: { src: artistAvatarUrl('jj-lin.webp'), position: '50% 38%', scale: 1.35 },
  孙燕姿: { src: artistAvatarUrl('stefanie-sun.webp'), position: '50% 24%', scale: 1.65 },
  邓紫棋: { src: artistAvatarUrl('gem.webp'), position: '50% 34%', scale: 1.45 },
  薛之谦: { src: artistAvatarUrl('joker-xue.webp'), position: '50% 29%', scale: 1.3 },
  汪苏泷: { src: artistAvatarUrl('silence-wang.webp'), position: '50% 27%', scale: 1.35 },
  梁静茹: { src: artistAvatarUrl('fish-leong.webp'), position: '50% 30%', scale: 1.4 },
  陶喆: { src: artistAvatarUrl('david-tao.webp'), position: '67% 46%', scale: 2.7 },
  王力宏: { src: artistAvatarUrl('wang-leehom.webp'), position: '50% 34%', scale: 1.35 },
  许嵩: { src: artistAvatarUrl('vae.webp'), position: '63% 25%', scale: 1.55 },
  陈奕迅: { src: artistAvatarUrl('eason-chan.webp'), position: '50% 43%', scale: 2.45 },
  郑润泽: { src: artistAvatarUrl('zheng-runze.webp'), position: '50% 22%', scale: 1.35 },
};
...
const SongRequestStation = ({ onBack }: SongRequestStationProps) => {
  ...
  const [sungStatsSource, setSungStatsSource] = useState<SungStatsSource>('requests');
  ...
  const sungCountMutationBusyRef = useRef(false);
  const voteSyncVersionRef = useRef(0);
  ...
  const [pendingRandomActive, setPendingRandomActive] = useState(false);
  const [shuffleTick, setShuffleTick] = useState(0);
  const [pendingSingCounts, setPendingSingCounts] = useState<Record<string, number>>(() => {
    if (typeof window === 'undefined') return {};
    try { const raw = window.localStorage.getItem(PENDING_SING_COUNTS_KEY); return raw ? JSON.parse(raw) : {}; } catch { return {}; }
  });
    ...
    const worker = (async () => {
      ...
      while (true) {
        ...
        try {
          ...
          showSyncMessage(mapArtistSettingsSyncError(error));
          return;
        }
      }
  ...
  useEffect(() => {
    ...
    if (location) {
      pullCloudVoteState(location).then(({ counts, sungCounts, requesterNames }) => {
        if (!active || version !== voteSyncVersionRef.current) return;
        setLocationVotes(counts);
        setLocationSungVotes(sungCounts);
  ...
  useEffect(() => {
    ...
    return () => {
      active = false;
      window.removeEventListener('jieyou-quiz-ranking-updated', refresh);
    };
  }, [rankingLocation]);
  ...
  const isPersonalRankingRandom = personalRankingMode === 'random';
  const personalRankingReversed = personalRankingMode === 'reverse';
  const visiblePersonalRanking = useMemo(() => {
    const base = personalRankingArtist
      ...
      : personalRanking;
    return orderPersonalRankingItems(base, personalRankingMode);
  }, [personalRanking, personalRankingArtist, personalRankingMode]);
  const paginatedPersonalRanking = useMemo(
  ...
  const songSearchResults = useMemo(() => {
    const needle = query.trim().toLowerCase();
    if (!needle) return [];
    return catalogSongs.filter((song) => song.title.toLowerCase().includes(needle));
  }, [catalogSongs, query]);
  ...
  useEffect(() => {
    setArtistPage((current) => Math.min(current, artistPageCount));
  }, [artistPageCount]);
  ...
  const requestSong = async (song: Song, explicitRequesterName?: string) => {
    ...
    try {
      ...
      if (rankingLocation !== '总榜') {
        pullCloudVoteState(rankingLocation).then(({ counts, sungCounts, requesterNames }) => {
          if (version !== voteSyncVersionRef.current) return;
          setLocationVotes(counts);
          setLocationSungVotes(sungCounts);
          setLocationVoteRequesters(requesterNames);
  ...
  const adjustRequestSungCount = async (songId: string, delta: 1 | -1 = 1) => {
    if (!songRecordSession || !canManageFeaturedSongs || sungCountMutationBusyRef.current) {
      if (!songRecordSession || !canManageFeaturedSongs) showSyncMessage('请先以站主账号进入我的档案后再记录已唱次数。');
      return;
    }
    sungCountMutationBusyRef.current = true;
    try {
      const location = rankingLocation === '总榜' ? undefined : rankingLocation;
      const synced = await adjustCloudSungVote(songRecordSession, songId, delta, location);
      setSungVotes(synced);
      if (location) {
        setLocationSungVotes((current) => {
          const nextCount = Math.max(0, (current[songId] ?? 0) + delta);
          const next = { ...current };
          if (nextCount) next[songId] = nextCount;
          else delete next[songId];
          return next;
        });
      }
      saveSungVoteCounts(window.localStorage, synced);
    } catch {
      showSyncMessage('已唱次数未同步，请检查网络后重试。');
    } finally {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (active) showSyncMessage('本设备原有的路演演唱次数暂未迁移，请检查网络后重试。');
      } finally {
        sungCountMutationBusyRef.current = false;
      }
    })();
  ...
  const finishAllRequestedSongs = async () => {
    ...
    try {
      ...
      // 若当前查看了具体地点，同步刷新地点分榜
      if (rankingLocation !== '总榜') {
        pullCloudVoteState(rankingLocation).then(({ counts, sungCounts }) => {
          if (version !== voteSyncVersionRef.current) return;
          setLocationVotes(counts);
          setLocationSungVotes(sungCounts);
        }).catch(() => {});
  ...
  const clearRequestedVotes = async (kind: 'pending' | 'sung') => {
    ...
    if (!window.confirm(`确定要清空总榜及所有地点的${label}吗？此操作不可恢复。`)) return;
    voteMutationBusyRef.current = true;
    ++voteSyncVersionRef.current;
    setClearingVotes(true);
    showSyncMessage('正在清除云端记录…');
    try {
      const synced = await clearCloudVotes(songRecordSession, kind);
      setVotes(synced.counts);
      setSungVotes(synced.sungCounts);
      if (kind === 'pending') setLocationVotes({});
      if (kind === 'sung') setLocationSungVotes({});
      saveVoteCounts(window.localStorage, synced.counts);
      saveSungVoteCounts(window.localStorage, synced.sungCounts);
      setPendingSingCounts({});
      try { window.localStorage.removeItem(PENDING_SING_COUNTS_KEY); } catch {}
      showSyncMessage(`已清空${label}，云端已同步`);
    } catch (error) {
      const code = error instanceof Error ? error.message : '';
      showSyncMessage(code === 'INVALID_ACTION'
        ? '云端清除功能尚未更新，原列表已保留。'
        : code === 'AUTH_FAILED' ? '管理口令失效，请重新进入私人记录后再清除。'
          : '清除未同步，原列表已保留，请检查网络后重试。');
    } finally {
      ++voteSyncVersionRef.current;
      voteMutationBusyRef.current = false;
  ...
  const updateQuizLevel = async (song: Song, level: QuizLevel) => {
    ...
    try {
      ...
      showSyncMessage('听歌识曲尚未同步，当前选择已保留');
    } finally {
      setQuizBusyId(null);
    }
  };
  ...
  const addSongToLatestRoadshow = async (song: Song) => {
    ...
    try {
      ...
      if (prepared.kind === 'duplicate') {
        setRoadshowArchives(records);
        showSyncMessage(`${song.title}已在最新路演的听歌识曲中`);
        return;
      }
      const saved = await saveRoadshow(songRecordSession, prepared.record);
      rememberRoadshowUpdate(records, saved);
      showSyncMessage(`${song.title}已加入“${saved.title}”的听歌识曲`);
    } catch {
      showSyncMessage('加入最新路演失败，请检查网络后重试');
    } finally {
      setRoadshowBusyId(null);
    }
  };

  const addQuizBatchToLatestRoadshow = async () => {
    if (!songRecordSession) {
      showSyncMessage('请先进入私人记录，再使用一键选歌');
      return;
    }
    if (roadshowBusyId) return;
    setRoadshowBusyId('quiz-batch');
    showSyncMessage('');
    try {
      const records = await pullRoadshows(songRecordSession);
      if (!getLatestRoadshow(records)) {
        setRoadshowArchives([]);
        showSyncMessage('还没有路演，请先在私人记录中创建一场路演');
        return;
      }
      const selection = selectQuizSongsForRoadshow(
        catalogSongs,
        quizAssignments,
        collectUsedRecognitionSongIds(records),
      );
      if (selection.kind === 'insufficient') {
        setRoadshowArchives(records);
        const detail = selection.shortages.map(({ level, required, available }) => {
          const label = QUIZ_LEVELS.find((item) => item.id === level)?.label ?? level;
          return `${label}需${required}首、剩${available}首`;
        }).join('；');
        showSyncMessage(`未使用歌曲不足：${detail}`);
        return;
      }
      const prepared = prepareLatestRoadshowRecognitionSongs(records, selection.songs);
      if (prepared.kind !== 'updated') {
        setRoadshowArchives(records);
        showSyncMessage(prepared.kind === 'missing' ? '还没有路演，请先创建' : '本轮歌曲均已加入最新路演');
        return;
      }
      const saved = await saveRoadshow(songRecordSession, prepared.record);
      rememberRoadshowUpdate(records, saved);
      showSyncMessage(`已按6:10:10:4选取30首并加入“${saved.title}”`);
    } catch {
      showSyncMessage('一键选歌失败，请检查网络后重试');
    } finally {
      setRoadshowBusyId(null);
    }
  };
  ...
  const addSongToLatestRoadshowPerformance = async (song: Song) => {
    ...
    try {
      ...
      if (prepared.kind === 'duplicate') {
        setRoadshowArchives(records);
        showSyncMessage(`${song.title}已在最新路演的路演歌曲中`);
        return;
      }
      const saved = await saveRoadshow(songRecordSession, prepared.record);
      rememberRoadshowUpdate(records, saved);
      setQuizMenuSongId(null);
      showSyncMessage(`${song.title}已加入“${saved.title}”的路演歌曲`);
    } catch {
      showSyncMessage('加入最新路演失败，请检查网络后重试');
    } finally {
      setRoadshowBusyId(null);
  ...
  const QuizLevelControl = ({ song }: { song: Song }) => {
    ...
    if (!songRecordSession) {
      return current ? <span title={`听歌识曲 · ${current.label}`} className={`inline-flex h-9 min-w-9 shrink-0 items-center justify-center rounded-full border px-2 text-[10px] font-black ${QUIZ_LEVEL_STYLES[current.id].badge}`}>{current.symbol}</span> : null;
    }
              ...
              <span className="grid grid-cols-2 gap-1">
                {QUIZ_LEVELS.map((level) => (
                  <button key={level.id} type="button" role="menuitemradio" aria-checked={currentLevel === level.id}
                    ...
                    <span className="ml-auto tabular-nums text-[10px] text-white/40">{quizCounts[level.id]}</span>
                  </button>
                ))}
              </span>
  ...
  const toggleFeaturedSong = async (song: Song) => {
    ...
    try {
      ...
      showSyncMessage('热门歌曲标记未同步，请稍后重试');
    } finally {
  ...
  return (
    <main className={`relative z-20 min-h-screen text-white ${popularImmersive ? 'h-screen overflow-hidden bg-transparent' : 'overflow-y-auto bg-[radial-gradient(circle_at_15%_0%,rgba(249,115,22,.14),transparent_30%),radial-gradient(circle_at_88%_18%,rgba(124,58,237,.12),transparent_28%)] px-4 py-5 sm:px-7 lg:px-10 lg:py-8'}`}>
      ...
      <div className="relative mx-auto w-full max-w-6xl">
        ...
        {selectedSong ? (
          <SongDetailPanel
            song={selectedSong}
            records={songRecords}
            roadshows={roadshowArchives}
            session={songRecordSession}
            syncStatus={recordSyncStatus}
            quizLevel={quizAssignments[selectedSong.id]}
            quizCounts={quizCounts}
            canManageQuiz={canManageFeaturedSongs}
              ...
              <button type="button" onClick={() => setActiveSection('quiz')}
                ...
                <div className="relative flex flex-col gap-6 sm:flex-row sm:items-center">
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <h2 className="mt-1 font-serif text-3xl font-black">听歌识曲</h2>
          ...
          <section>
            ...
            <div className="mb-7 flex items-end justify-between gap-4">
              ...
              {activeSection === 'quiz' && (
                <button type="button" onClick={() => { void addQuizBatchToLatestRoadshow(); }} disabled={Boolean(roadshowBusyId)}
                  title="简单6 · 常规10 · 较难10 · 很难4" aria-label="一键选歌：简单6首、常规10首、较难10首、很难4首"
                  className="inline-flex shrink-0 items-center gap-2 rounded-full border border-sky-200/25 bg-sky-300/10 px-4 py-2.5 text-xs font-black text-sky-100 shadow-[0_10px_30px_rgba(56,189,248,.08)] transition hover:border-sky-100/45 hover:bg-sky-300/20 disabled:cursor-wait disabled:opacity-45">
              ...
              <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
                  ...
                  {visibleRankingView === 'requests' ? (
                      ...
                      {requestVoteView === 'pending' ? (
                          ...
                          <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain pr-2">
                            <ol className="space-y-3">{displayRanking.map(({ song, count, requesters }, index) => (
                              <li key={song.id} className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/[.035] p-4">
                                ...
                                {(() => {
                                  const sungCount = requestSungCounts[song.id] ?? 0;
                                  return <div className="flex shrink-0 items-center gap-1.5">
                                    <button type="button" aria-label={`${song.title}点歌已唱次数减1`} title="撤回1次点歌已唱" disabled={!canManageFeaturedSongs || sungCount <= 0} onClick={() => void adjustRequestSungCount(song.id, -1)} className="grid h-7 w-7 place-items-center rounded-full border border-orange-200/20 bg-orange-300/10 text-sm font-bold text-orange-200 transition enabled:hover:bg-orange-300/20 disabled:cursor-default disabled:opacity-25">−</button>
                                    <button type="button" aria-label={`${song.title}点歌已唱次数加1`} disabled={!canManageFeaturedSongs} onClick={() => void adjustRequestSungCount(song.id)} title="把这首游客已点歌曲记入点歌已唱" className="shrink-0 rounded-xl border border-orange-200/20 bg-orange-300/10 px-3 py-1.5 text-xs font-black text-orange-200 transition hover:bg-orange-300/20 hover:text-orange-100 disabled:cursor-not-allowed disabled:opacity-35">已唱{sungCount}次</button>
                                  </div>;
                                })()}
                              </li>
                            ))}</ol>
                          </div>
                          {canManageFeaturedSongs && displayRanking.length > 0 && (
                            <div className="flex shrink-0 justify-center pt-4">
                              <button type="button" disabled={finishingVotes || clearingVotes} onClick={() => void finishAllRequestedSongs()} className="inline-flex items-center justify-center gap-2 rounded-full border border-orange-200/35 bg-orange-300 px-7 py-3 text-sm font-black text-black transition hover:bg-orange-200 disabled:cursor-wait disabled:opacity-55">
                        ...
                        sungArtistRanking.length ? <div className={sungArtistRanking.length > REQUEST_RANKING_SCROLL_THRESHOLD ? 'max-h-[42rem] overflow-y-auto overscroll-contain pr-2' : ''}><ol className="space-y-3">{sungArtistRanking.map(({ artist, count, songCount }, index) => (
                          <li key={artist} className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/[.035] p-4">
                            <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-full font-serif font-black ${RANKING_MEDAL_CLASSES[getRankingMedalTone(index, 3)]}`}>{index + 1}</span>
                            <span className="min-w-0 flex-1"><strong className="block truncate font-bold">{artist}</strong><small className="block truncate text-xs text-white/40">{songCount} 首歌进入{sungStatsSource === 'roadshow' ? '路演演唱' : '点歌已唱'}</small></span>
                        ...
                        ))}</ol></div> : <div className="grid min-h-64 place-items-center text-center text-white/40"><div><Mic2 className="mx-auto h-9 w-9 opacity-40" /><p className="mt-3">{sungStatsSource === 'roadshow' ? '还没有路演演唱歌手记录' : '还没有点歌已唱歌手记录'}</p></div></div>
                      ) : (
                        ...
                        ))}</ol></div> : <div className="grid min-h-64 place-items-center text-center text-white/40"><div><Trophy className="mx-auto h-9 w-9 opacity-40" /><p className="mt-3">{sungStatsSource === 'roadshow' ? '还没有路演演唱歌曲记录' : '还没有点歌已唱歌曲记录'}</p></div></div>
                      )}
                    </div>
                  ) : visibleRankingView === 'personal' ? (
                    visiblePersonalRanking.length ? <>
                      <ol
                        ...
                        tabIndex={paginatedPersonalRanking.items.length > PERSONAL_RANKING_SCROLL_THRESHOLD ? 0 : undefined}
                        aria-label={paginatedPersonalRanking.items.length > PERSONAL_RANKING_SCROLL_THRESHOLD ? `${personalRankingArtist ?? '总榜'}${isPersonalRankingRandom ? '随机' : personalRankingReversed ? '倒序' : '匹配度'}排行，可上下滚动` : undefined}
                      ...
                      {paginatedPersonalRanking.pageCount > 1 && (
                        <nav aria-label="练习榜分页" className="mt-5 flex flex-wrap items-center justify-center gap-3 text-xs text-white/45">
                          <span>共 {paginatedPersonalRanking.total} 首</span>
                          <span className="flex items-center gap-2 rounded-full border border-white/10 bg-black/30 p-1">
                            <button type="button" aria-label="上一页练习榜" disabled={paginatedPersonalRanking.page === 1} onClick={() => setPersonalRankingPage((current) => Math.max(1, current - 1))} className="grid h-8 w-8 place-items-center rounded-full text-white/60 transition hover:bg-white/10 hover:text-white disabled:cursor-not-allowed disabled:opacity-20"><ChevronLeft className="h-4 w-4" /></button>
                ...
                {visibleRankingView === 'requests' ? (
                  <aside data-request-ranking-controls className="h-fit rounded-[1.75rem] border border-orange-200/15 bg-orange-950/20 p-4 sm:p-5">
                    ...
                    {requestVoteView === 'sung' && (
                      <div role="tablist" aria-label="已唱数据来源切换" className="mt-2 grid grid-cols-2 gap-2">
                        <button type="button" role="tab" aria-selected={sungStatsSource === 'requests'} onClick={() => setSungStatsSource('requests')} className={`rounded-xl border px-3 py-2.5 text-xs font-black transition ${sungStatsSource === 'requests' ? 'border-orange-300/45 bg-orange-300 text-black' : 'border-white/10 bg-black/25 text-white/65 hover:text-white'}`}>点歌已唱</button>
                        <button type="button" role="tab" aria-selected={sungStatsSource === 'roadshow'} onClick={() => setSungStatsSource('roadshow')} className={`rounded-xl border px-3 py-2.5 text-xs font-black transition ${sungStatsSource === 'roadshow' ? 'border-orange-300/45 bg-orange-300 text-black' : 'border-white/10 bg-black/25 text-white/65 hover:text-white'}`}>路演演唱</button>
                      </div>
                    )}
                    ...
                    {requestVoteView === 'pending' && canManageFeaturedSongs && (
                      <div className="mt-2">
                        <button type="button" disabled={ranking.length === 0 || roadshowBusyId === 'import-pending'} onClick={async () => {
                          if (!songRecordSession || ranking.length === 0) return;
                          setRoadshowBusyId('import-pending');
                          showSyncMessage('');
                          try {
                            const records = await pullRoadshows(songRecordSession);
                            const latest = records.sort((a, b) => b.date.localeCompare(a.date) || b.updatedAt.localeCompare(a.updatedAt))[0];
                            if (!latest) {
                              showSyncMessage('还没有路演，请先在私人记录中创建一场路演');
                              return;
                            }
                            const additions = ranking.map(({ song }) => createRoadshowSong(song));
                            if (additions.length === 0) {
                              showSyncMessage('已点列表为空，无可导入的歌曲');
                              return;
                            }
                            const performanceSongs = deduplicateRoadshowSongs([...latest.performanceSongs, ...additions]);
                            const addedCount = performanceSongs.length - deduplicateRoadshowSongs(latest.performanceSongs).length;
                            const updated: RoadshowRecord = { ...latest, performanceSongs, updatedAt: new Date().toISOString() };
                            const saved = await saveRoadshow(songRecordSession, updated);
                            rememberRoadshowUpdate(records, saved);
                            // 通知 RoadshowPanel 刷新编辑缓存，确保用户切回时看到新数据
                            window.dispatchEvent(new CustomEvent('jieyou-roadshow-imported', { detail: { recordId: saved.id } }));
                            showSyncMessage(addedCount > 0 ? `已将 ${addedCount} 首已点歌曲导入「${saved.title}」的路演歌曲` : '这些歌曲已在本场路演中，无需重复导入');
                          } catch {
                            showSyncMessage('导入失败，请检查网络后重试');
                          } finally {
                            setRoadshowBusyId(null);
                          }
                        }} className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-orange-200/25 bg-orange-300/10 px-3 py-2.5 text-xs font-black text-orange-100 transition hover:bg-orange-300/20 disabled:cursor-not-allowed disabled:opacity-30">
                          <Upload className="h-3.5 w-3.5" />{roadshowBusyId === 'import-pending' ? '导入中…' : '一键导入'}
                        </button>
                      </div>
                    )}
                  ...
                  <aside aria-label="练习榜歌手筛选" className="h-fit overflow-hidden rounded-[1.75rem] border border-orange-200/15 bg-orange-950/20 p-4 sm:p-5">
                    <div className="grid grid-cols-2 gap-2">
                      <label className="flex h-11 min-w-0 items-center gap-2 rounded-xl border border-white/10 bg-black/35 px-3 focus-within:border-orange-300/45">
                        ...
                        <input value={rankingArtistQuery} onChange={(event) => setRankingArtistQuery(event.target.value)} placeholder="搜索歌手或歌曲" className="min-w-0 flex-1 bg-transparent text-xs text-white outline-none placeholder:text-white/28" />
                      </label>
                      <button
                        type="button"
                        aria-label="切换练习榜正倒序"
                        aria-pressed={personalRankingReversed}
                        onClick={() => { setPersonalRankingMode(togglePersonalRankingReverse); setPersonalRankingPage(1); }}
                        className={`inline-flex h-11 items-center justify-center gap-1.5 rounded-xl border px-3 text-xs font-black transition ${personalRankingReversed ? 'border-orange-200/40 bg-orange-300 text-black' : 'border-white/10 bg-black/30 text-white/55 hover:text-white'}`}
                      >
                        <ArrowUpDown className="h-4 w-4" />{personalRankingReversed ? '倒序' : '正序'}
                      </button>
                    </div>
                    <div className="mt-3 flex gap-2">
                      ...
                      <button type="button" aria-pressed={isPersonalRankingRandom} onClick={() => { setPersonalRankingMode(togglePersonalRankingRandom); setPersonalRankingPage(1); }} className={`flex-1 rounded-xl border px-4 py-3 text-sm font-black transition ${isPersonalRankingRandom ? 'border-orange-300/45 bg-orange-300 text-black' : 'border-white/10 bg-black/25 text-white/65 hover:border-orange-200/25 hover:text-white'}`}>随机</button>
                    </div>
                  ...
                  <aside data-quiz-ranking-controls className="h-fit rounded-[1.75rem] border border-sky-200/15 bg-sky-950/20 p-4 sm:p-5">
                    ...
                    <section key={level.id} className={`flex h-[22rem] flex-col rounded-[1.75rem] border p-5 sm:p-6 ${QUIZ_LEVEL_STYLES[level.id].panel}`}>
                      ...
                      {levelSongs.length ? (
                        <div className="min-h-0 flex-1 grid grid-cols-1 gap-2 overflow-y-auto overscroll-contain sm:grid-cols-2 pr-1 content-start">
                          {levelSongs.map((song) => {
                            const used = usedRecognitionSongIds.has(song.id);
                            return (
                              <article key={song.id} className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/25 p-3">
                                <button type="button" onClick={() => openSongDetail(song)} className="min-w-0 flex-1 text-left">
                                ...
                                {used && (
                                  <span title="已用于往期路演，不参与下次一键选歌" className="inline-flex shrink-0 items-center gap-1 rounded-full border border-white/10 bg-white/[.055] px-2 py-1 text-[10px] font-bold text-white/40">
              ...
              <div className="space-y-5">
                ...
                {selectedArtist ? (
                  ...
                  <>
                    ...
                    <div className={`grid content-start gap-3 sm:grid-cols-2 lg:grid-cols-4 ${artistOrderMode ? '' : 'lg:min-h-[34.5rem]'}`}>{paginatedArtistGroups.map(({ artist, songs }, index) => {
                        const avatar = getArtistAvatar(artist);
                        const avatarStyle = getAvatarStyle(artist);
                      ...
                      {!artistOrderMode && artistGroups.length > ARTISTS_PER_PAGE && (
                        <nav aria-label="歌手分页" className="mt-5 flex flex-wrap items-center justify-center gap-3 text-xs text-white/45">
                          ...
                          <span className="flex items-center gap-2 rounded-full border border-white/10 bg-black/30 p-1">
                            ...
                            <strong className="min-w-14 text-center font-bold text-white/70">{artistPage} / {artistPageCount}</strong>
                            <button
                              type="button"
                              aria-label="下一页歌手"
      ...
      {syncMessage && (
        <div className="fixed bottom-5 left-1/2 flex -translate-x-1/2 items-center gap-2 rounded-full border border-red-200/15 bg-black/85 py-2 pl-4 pr-2 text-xs text-red-100 shadow-xl">
