...
export const hasCustomArtistSettings = (payload: ArtistSettingsPayload, defaultArtistOrder: string[]) => (
...
export const resolveArtistSettingsPull = ({ cloud, local, draft, hasSession, defaultArtistOrder }: {
  ...
  if (hasSession && hasCustomArtistSettings(local, defaultArtistOrder)) return { kind: 'seed-cloud' as const, payload: local };
